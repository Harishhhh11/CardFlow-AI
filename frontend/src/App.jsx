  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {currentStep === "success" && (
          <SuccessScreen contact={contact} savedContactName={savedContactName} onNewScan={resetApplication} />
        )}
      </main>
    ...
    <section className="screen screen-upload screen-enter">
      <div className="hero-section">
        <div className="hero-copy">
          <div className="hero-chip"><span>✦</span> AI VISITING CARD SCANNER</div>
          <h2>Turn every card into a <span>smart contact.</span></h2>
          <p>Capture a visiting card, let CardFlow AI extract the details, review the result, and save a clean contact in moments.</p>
          <div className="hero-features">
            <div><span>✦</span>AI extraction</div>
            <div><span>✓</span>Review before save</div>
            <div><span>⚡</span>Fast processing</div>
          </div>
          <div className="hero-visual-note"><span className="hero-visual-dot" /> Built for fast contact capture</div>
        </div>
        <div className="hero-visual" aria-hidden="true">
          <div className="hero-card-stack card-back" />
          <div className="hero-card-stack card-middle" />
          <div className="hero-ai-card">
            <div className="ai-card-top"><div className="mini-profile"><span /></div><div><strong>Contact detected</strong><small>Ready for review</small></div><div className="ai-pulse">✦</div></div>
      ...
      <section className="scanner-panel home-capture-panel">
        <div className="panel-heading advanced-panel-heading">
          <div>
            <div className="section-number"><span>01</span><small>CAPTURE</small></div>
            <h2>Bring in your card</h2>
            <p>Front side is required. Add the back side when it contains more contact information.</p>
          </div>
          <div className="capture-trust"><strong>Private by design</strong><span>Images are only used for extraction.</span></div>
        </div>

        <div className="scanner-grid">
          <CardUploader title="Front Side" required preview={side1Preview} onFileChange={(file) => onFileChange(file, "side1")} onOpenCamera={() => onOpenCamera("side1")} onRemove={() => onRemove("side1")} />
        ...
        <div className="scan-footer">
          <div className="scan-security"><div className="security-icon">🔒</div><div><strong>Ready when you are</strong><p>JPG, PNG or WEBP up to 10 MB.</p></div></div>
          <button type="button" className="scan-ai-button" onClick={onContinue} disabled={!side1Preview}><span>Scan with CardFlow AI</span><span className="button-arrow">→</span></button>
        </div>
      </section>
    </section>
  );
      ...
      <div className="processing-card processing-card-enhanced">
        <div className="processing-visual processing-visual-enhanced">
          ...
          <div className="processing-card-preview">
            ...
            <div className="scan-corners"><i className="corner tl" /><i className="corner tr" /><i className="corner bl" /><i className="corner br" /></div>
          </div>
        ...
        <div className="processing-content processing-content-enhanced">
          ...
          <div className="extraction-progress"><div className="extraction-progress-top"><span>Extraction progress</span><strong>{Math.round(progress)}%</strong></div><div className="extraction-progress-track"><span style={{ width: `${progress}%` }} /></div></div>
          <div className="extraction-steps">
            {PROCESSING_STEPS.map((step, index) => {
              const completed = index < processingIndex;
              const current = index === processingIndex;
              return <div className={`extraction-step ${completed ? "completed" : ""} ${current ? "current" : ""}`} key={step.title}><div className="extraction-step-marker">{completed ? "✓" : String(index + 1).padStart(2, "0")}</div><div className="extraction-step-copy"><strong>{step.title}</strong><small>{current ? "Processing now" : completed ? "Completed" : "Waiting"}</small></div>{current && <div className="step-pulse" />}</div>;
            })}
          </div>
...
function ReviewScreen({ contact, side1Preview, side2Preview, error, onBack, onSave, updateField, updateListField, addListItem, removeListItem }) {
  const scalarFields = [
    ["name", "Full Name", "The person's name"], ["company_name", "Company", "Company or organization"], ["designation", "Designation", "Job title or role"],
    ["website", "Website", "Company or personal website"], ["linkedin", "LinkedIn", "LinkedIn profile URL"], ["address", "Address", "Address or office location"], ["other_details", "Other Details", "Any additional information"]
  ];

  return (
    <section className="screen-review screen-enter">
      <div className="review-header">
        <div className="review-title"><div className="section-number"><span>02</span><small>REVIEW</small></div><h2>Polish the contact before saving</h2><p>AI has structured the information. Check anything that needs a correction.</p></div>
        <div className="review-confidence"><span className="confidence-dot" />AI extraction complete</div>
      </div>
      <div className="review-layout">
        <aside className="review-card-preview"><div className="preview-card-label">SOURCE IMAGE</div><div className="review-image-wrap">{side1Preview && <img src={side1Preview} alt="Front side of visiting card" />}</div>{side2Preview && <div className="review-image-wrap review-back-preview"><img src={side2Preview} alt="Back side of visiting card" /></div>}<div className="preview-note">Compare the original card with the extracted fields before saving.</div></aside>
        <div className="editor-panel">
          ...
          <div className="save-contact-footer"><button type="button" className="secondary-button" onClick={onBack}>← Back to scan</button><button type="button" className="save-contact-button" onClick={onSave}><span>Save Contact</span><span>✓</span></button></div>
        </div>
      </div>
    </section>
  );
